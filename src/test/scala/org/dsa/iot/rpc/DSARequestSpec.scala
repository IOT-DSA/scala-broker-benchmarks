 ...
 */
class DSARequestSpec extends AbstractSpec with GeneratorDrivenPropertyChecks {
  import DSAMethod._
  import DSAValue._
  ...
  "InvokeRequest" should {
    ...
    "serialize with complex parameters to JSON" in forAll { (a: String, b: Boolean, c: Int, d: Double) =>
      ...
      val bcd = MapValue(Map("b" -> b, "cd" -> cd))
      testJson(
        InvokeRequest(Rid, Path, Map("bcd" -> bcd, "a" -> a)),
  ...
  "UnsubscribeRequest" should {
    "serialize to JSON" in testJson(
      UnsubscribeRequest(Rid, ScalaList(1, 2, 3)),
